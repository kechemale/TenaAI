...
sys.path.append(os.path.join(os.path.dirname(__file__), "src"))
from src.vectorstore import FaissVectorStore
from src.search import RAGSearch, _append_eval_to_gsheet
...
if prompt := st.chat_input("Ask a healthcare-related question..."):
    ...
    with st.chat_message("assistant"):
        ...
        with st.spinner("🔍 Retrieving and summarizing relevant medical information..."):
            try:
                # Get both response and retrieved contexts
                raw_response, retrieved_contexts = rag_search.search_and_summarize(
                    prompt, top_k=3, return_contexts=True
                )
                # Extract text from your LLM
                answer = raw_response  # response.choices[0].message.content inside search_and_summarize
                response = format_clinical_response(answer)
            except Exception as e:
                response = f"⚠️ **Error**: {str(e)}"
                retrieved_contexts = []

        # Stream with character-level animation
    ...
    try:
        _append_eval_to_gsheet(
            query=prompt,
            top_k=len(retrieved_contexts),
            contexts=retrieved_contexts,
            response=full_response,
            sheet_name="TenaAI_Logs"
        )
    except Exception as e:
        st.error(f"⚠️ Failed to log to Google Sheet: {e}")
